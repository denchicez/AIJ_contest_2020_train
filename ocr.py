    ...
    with torch.no_grad():
        for filename in os.listdir(hp.test_dir):
            ...
            with open(os.path.join('/output', filename.replace('.jpg', '.txt').replace('.png', '.txt')), 'w', encoding="utf-8") as file:
                ...
                for slovo in razdel:
                    if(proshloe_slovo!=''):
                        new_slovo = proshloe_slovo+slovo
                        if(new_slovo in words):
                            first = pred.find(proshloe_slovo+' '+slovo, 0,len(pred))
                            second=len(proshloe_slovo)+len(slovo)+1+first
                            pred = pred[:first]+new_slovo+pred[second:]
                            proshloe_slovo=''
                        else:
                            proshloe_slovo=slovo
                    else:
                        proshloe_slovo=slovo
                file.write(pred)
